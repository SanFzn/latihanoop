...
class Mengajar {
    ...
    //READ - get all teaching assignments with joins
    ...
    //READ BY GURU ID - get teaching assignments for a specific teacher
    ...
    //DELETE BY GURU ID
    public function deleteByGuruId($guru_id) {
        $query = "DELETE FROM $this->table WHERE guru_id=?";
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param("i", $guru_id);
        return $stmt->execute();
    }
    ...
    // GET DATA + LIMIT
    public function getAllWithLimit($limit, $offset, $keyword = null) {
        if ($keyword) {
            $query = "SELECT m.id,
                            g.nama AS guru,
                            mp.nama AS mapel,
